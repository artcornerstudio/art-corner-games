import type { Point, Variant } from "../types/play";
...
export function makeViewport(variant: Variant, containerPx: number): Viewport {
  const spec = FIELD_SPECS[variant];
  const scale = Math.max(containerPx / spec.width, MIN_SCALE);
  return { spec, scale, widthPx: Math.round(spec.width * scale), heightPx: Math.round(VIEW_DEPTH * scale) };
}
